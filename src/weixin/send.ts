...
/** Summarize a multi-segment send. allSent is true iff every segment was sent. */
export function deliveryResult(total: number, sent: number): DeliveryResult {
  return { total, sent, allSent: total === sent };
}
...
export async function sendTextMessage(
  ...
  const id = clientId ?? `wechat-acp-codex-${crypto.randomUUID()}`;
  await sendFn({
