/**
 * Configuration types and defaults for wechat-acp-codex.
 */
...
/**
 * Canonical bridge slash commands that `wechat-acp-codex` handles itself
...
export interface WeChatAcpConfig {
  ...
  owner?: { id?: string; allowFirst?: boolean };
  session: {
 ...
 * `~/.wechat-acp-codex/instances/`, so we restrict them to a safe character set
...
export function defaultStorageDir(instance?: string): string {
  const root = path.join(os.homedir(), ".wechat-acp-codex");
...
export function defaultConfig(opts?: { instance?: string }): WeChatAcpConfig {
  ...
  return {
    ...
    daemon: {
      enabled: false,
      logFile: path.join(storageDir, "wechat-acp-codex.log"),
...
/**
 * Decide which agent selection the CLI should resolve, defaulting to a preset
 * only when neither --agent, a config preset, nor a raw config command is
 * present. Returning `undefined` means "a raw config.command is in effect —
 * do not override it".
 */
export function resolveCliAgentSelection(opts: {
  argAgent?: string;
  configPreset?: string;
  configCommand?: string;
  defaultPreset: string;
}): string | undefined {
  const selection = opts.argAgent ?? opts.configPreset;
  if (selection) return selection;
  if (opts.configCommand) return undefined;
  return opts.defaultPreset;
}
