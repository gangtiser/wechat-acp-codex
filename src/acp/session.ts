...
/** A turn is ack-able iff the agent didn't die and every gated send delivered. */
export function turnSettledOk(input: { agentDied: boolean; deliveryFailed: boolean }): boolean {
  return !input.agentDied && !input.deliveryFailed;
}
...
export interface SessionManagerOpts {
  ...
  showDiffs?: boolean;
  log: (msg: string) => void;
  onReply: (userId: string, contextToken: string, text: string) => Promise<DeliveryResult>;
  onTurnSettled?: (inboxId: string, ok: boolean) => void | Promise<void>;
    ...
    try {
      while (session.queue.length > 0 && !this.aborted) {
        ...
        const reply = async (text: string): Promise<void> => {
          const r = await this.opts.onReply(session.userId, pending.contextToken, text);
          if (!r.allSent) deliveryFailed = true;
        };
        ...
        try {
          ...
          if (replyText.trim()) {
            ...
            await reply(emptyTurnNotice(result.stopReason));
          }
        } catch (err) {
          completionError = err;
          ...
          try {
            await reply(`⚠️ Agent error: ${String(err)}`);
          } catch {
            // best effort
          }
        } finally {
          if (pending.inboxId) {
            void Promise.resolve(
              this.opts.onTurnSettled?.(pending.inboxId, turnSettledOk({ agentDied, deliveryFailed })),
            ).catch((e) => this.opts.log(`[${session.userId}] inbox settle (turn) failed: ${String(e)}`));
          }
